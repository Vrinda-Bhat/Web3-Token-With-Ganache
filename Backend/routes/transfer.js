...
router.post('/WalletTransfer',async(req,res)=>{
   ...
   await contract.methods.transfer(account2, amount).send({ from: account1 })
   const result=web3.utils.fromWei(amount,'ether')
   res.json({msg:`Transferred Successfully ${result} Points`})
  //  res.json(`Transferred Successfully ${web3.utils.fromWei(amount, 'ether')} Points`)
